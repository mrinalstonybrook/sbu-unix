...
printk("\n sata_fs_count = [%d]",sata_fs_count);
	...
	for(i=0;i<sata_fs_count;i++){
                cmp_buf[0]='\0';
		slash_len=upto_last_slash_len(sata_fs[i].name);
		strncpy(cmp_buf, sata_fs[i].name, (slash_len));
                printk("\ncmpbuf=[%s],sata_fs[i].name=[%s]",cmp_buf,sata_fs[i].name);
		for(j=0;j<sata_fs_count;j++){
			if(i==j)
				continue;
			if (strncmp(cmp_buf,sata_fs[j].name,strlen(sata_fs[j].name)) == 0){
				if(sata_fs[j].typeflag == DIRECTORY){
					sata_fs[j].number_of_dentry +=1;
					sata_fs[i].par_ind=j;
                                        printk("\n par_ind %d child %d",j,i);
					break;
				}
			}
		}
...
////// DEFAULT DIRECTORY ///////////////
	/*slash_len=0;
	for(i=0;i<sata_fs_count;i++){
		if (strcmp("/hiphop",sata_fs[i].name) == 0){
			slash_len=1;
			break;
		}
	}
...
/*int dir_read(struct file *dd){
	int i=0;
	char cmp_buf[100];
	strcpy(cmp_buf,dd->filename);
	for(i=0;i<sata_fs_count;i++){
		if(strcmp(dd->filename,sata_fs[i].name)==0)
			continue;
		if(compare(sata_fs[i].name,dd->filename,strlen(cmp_buf)+2))
			printk("\n[%d] [%s]",i,sata_fs[i].name);
	}
	if(i==sata_fs_count)
		return 0;
	return 1;
...
int dir_read(struct file * dir,char *buf,int size){
	...
	for(i=0;i<sata_fs_count;i++){
		...
		if(flag==1)
		{
			...
			//strcpy(buf+k,sata_fs[i].name);
			//k=k+strlen(sata_fs[i].name);
			buf[count++]=',';
			 //k=k+1;
			flag=0;

...
void set_page_free(uint64_t page)
{
	if (page)
        ...
        page_ref[(uint64_t)page / 8] &= ~(1 << (page % 8));
...
{
    int num = size_mem/PAGE_SIZE + 1;
    uint64_t temp_viraddr = virt_addr & PAGE_ALIGN ; 
    uint64_t* phys_addr=NULL;
    uint32_t  c;
    bool isBase = 0;
    uint64_t base_addr = NULL;
	...
	if(!isBase){
		base_addr = temp_viraddr;
		isBase = 1;
	}
    	phys_addr = kmalloc_mem();
    	mmap((uint64_t)temp_viraddr,(uint64_t)phys_addr);  
       //memset((void *)(temp_viraddr & PAGE_ALIGN), 0, PAGE_SIZE);   
    	temp_viraddr = temp_viraddr + PAGE_SIZE;
    }
...
void* kmalloc_iden(uint64_t size_mem)
{
    ...
    uint32_t  c;
    bool isBase = 0;
    uint64_t *base_addr_phy = NULL;
    

    for(c=0;c<num;++c){
       phys_addr = kmalloc_mem();
	virt_addr = PHY_TO_VIRT(phys_addr,void *);
	
	if(!isBase){
		base_addr_phy = phys_addr;
		isBase = 1;
	}
...
{
    int num = size_mem/PAGE_SIZE;
	
    uint64_t virt_addr=NULL;
    uint32_t  c;
    bool isBase = 0;
    uint64_t base_addr = NULL;
    phys_addr = phys_addr & PAGE_ALIGN;
    uint64_t page = phys_addr >> 12;
      
    for(c=0;c<num;++c){
        if( page < MAX_PAGES && !(free_pages_list[(uint64_t)page / 8] >> (page % 8)) & 0x01)
		{
			free_pages_list[(uint64_t)page / 8] |= 1 << (page % 8);
		}
	virt_addr = phy_to_virtual(phys_addr);

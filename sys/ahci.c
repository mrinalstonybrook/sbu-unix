	...
	while (i<32){
		if (pi & 1){
			...
			if (dt == AHCI_DEV_SATA){
				...
				abar = abar_temp;
	//			printk("\nafter%d",((HBA_PORT *)&abar_temp->ports[i])->tfd );
//				printk("\nafter%d",((HBA_PORT *)&abar_temp->ports[i])->ssts);
//				printk("\nafter%d",((HBA_PORT *)&abar_temp->ports[i])->ie);
//				printk("\nafter%d",((HBA_PORT *)&abar_temp->ports[i])->serr );
//				printk("\nafter%d",((HBA_PORT *)&abar_temp->ports[i])->is );


				port_rebase(abar_temp->ports, i);
		//		printk("\ntfd %d",((HBA_PORT *)&abar_temp->ports[i])->tfd );
	//			printk("\nssts %d",((HBA_PORT *)&abar_temp->ports[i])->ssts);
	//			printk("\nie %d",((HBA_PORT *)&abar_temp->ports[i])->ie);
	//			printk("\nserr %d",((HBA_PORT *)&abar_temp->ports[i])->serr );
		//		printk("\nis%d",((HBA_PORT *)&abar_temp->ports[i])->is );

		//		read(&abar_temp->ports[0], 0, 0,10 , (uint64_t)pages_for_ahci_start + (20*4096)/8);
	//			uint64_t addres=0;
		//		addres=(uint64_t)pages_for_ahci_start + (20*4096)/8;
			//	addres =  addres+ KERNBASE;
			//	printk("\nDATA [::");
                         //       char *tmp=(char *)(addres);
                          //      for(i=0;i<10;i++)
		//		printk("%c",tmp[i]);
		//		printk("\n");

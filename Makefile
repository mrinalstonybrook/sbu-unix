...
submit: clean
	tar -czvf $(USER).tgz --exclude=$(ROOTLIB) --exclude=$(ROOTBIN) --exclude=.empty --exclude=.*.sw? --exclude=*~ LICENSE Makefile linker.script sys bin libc ld superblock hdd.img initiate_superblock.c README_SBUNIX include $(ROOTFS)

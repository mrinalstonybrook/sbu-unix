...
char * convert_to_string(uint64_t value, char * str, int base, char *str_tmp )
{
		...
		if(*format !='%' && *format!='\t' && *format!='\n' ){
			...
			switch(*(format+1)){
				case 'd':
					...
					number = va_arg(args, int);
						//number++;
                                        convert_to_string(number,str_tmp,10,str_tmp);
                                        tmp_c=str_tmp;
				...
				case 'x':
					number2 = va_arg(args, uint64_t);
                                        convert_to_string(number2,str_tmp,16,str_tmp);
                                        tmp_c=str_tmp;
					while(*tmp_c!='\0'){
						string_buffer[tmp]=*tmp_c;
                                                tmp_c++;
...
////////////////////////
//char *test = "Sending string from userspace";
__syscall4(10, (uint64_t)string_buffer, 0, 0, 0);	
return 0;
